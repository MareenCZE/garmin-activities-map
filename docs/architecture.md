...
- **Incremental FTP upload** (`upload_map_with_data_to_ftp_incremental`): compares local vs
  ...
  clean-then-full).
- **FTP password** is stored encrypted in config using Fernet; however the key
  (`CRYPTO_KEY`) is **hard-coded in `ftpuploader.py`**, so this is obfuscation, not real
  security. Auth tokens for Garmin persist in `.auth/`.
...
main rough edges are the obsolete minification artefacts, the hard-coded FTP crypto key, and
